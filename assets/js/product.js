...
function createProductCard(product) {
  ...
  const cardContent = `
        ...
        <div class="card-body bg-body-tertiary">
            <h6 class="card-title about-sub-heading fw-bold">${
              product.nama
            }</h6>
            <div class="row">
            ...
            <div class="col-lg-6 col-md-12 col-sm-6 col-6 text-lg-end text-md-start text-sm-end text-end">
                <span class="fw-bold">Rp ${product.harga.toLocaleString(
                  "id-ID"
                )}</span>
            </div>
            </div>
            <button type="button" class="btn btn-warning rounded-0 w-100 text-dark fw-semibold" data-bs-toggle="modal" data-bs-target="#productModal" onclick="showProductModal(${
              product.id
            })">
      ...
      if (product) {
        ...
        modalBody.innerHTML = `
                    <div class="modal-body d-flex align-items-center">
                        <img src="${
                          product.gambar
                        }" style="width: 50%;" class="img-fluid mb-3 me-3" alt="${
          product.nama
        }">
                        <div>
                            <h5>${product.nama}</h5>
                            <span class="fw-bold btn btn-warning">Rp ${product.harga.toLocaleString(
                              "id-ID"
                            )}</span>
        ...
        const whatsappLink = `https://wa.me/+6285659573835/?text=Saya%20ingin%20membeli%20${
          product.nama
        }%20dengan%20harga%20${product.harga.toLocaleString("id-ID")}%0A%0A${
          product.deskripsi
        }`;
  ...
  fetch("https://aidinaagustin.github.io/json/products.json")
    ...
    .then((data) => {
      const filteredProducts = data.products.filter(
        (product) => product.kategori.toLowerCase() === category.toLowerCase()
      );
